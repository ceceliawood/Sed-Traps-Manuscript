...
CTD_casts <- EDI_CTD |>
  ...
  distinct(Reservoir, Date, .keep_all = TRUE)
...
# Make a vector of sampling days so CTD and handhelds do not overlap
  CTD_days <- EDI_CTD %>% 
    select(Date) %>% 
    unique()
  
# add in handhelds to supplement data gaps
  Handhelds  <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/198/13/e50a50d062ee73f4d85e4f20b360ce4f") %>% 
    filter(between(DateTime, as.Date('2018-01-01 00:00:00'),
                   as.Date('2023-12-31 23:59:59')),
           Reservoir == 'FCR' | Reservoir == 'BVR',
           Site == 50,
           Depth_m >= 0) %>% 
    arrange(DateTime)
...
Handhelds1 <- Handhelds %>% 
  mutate(Date = date(DateTime),
         Year = year(DateTime)) %>% 
  filter(Date %!in% CTD_days$Date) %>% 
  select(Reservoir, Site, DateTime, Depth_m, Temp_C, DO_mgL, Date, Year) %>% 
  #fixing leap day issue, shifting sample date by one day
  mutate(Date = if_else(Date == as.Date('2020-02-29'), 
                          as.Date('2020-02-28'), Date))

# Make a df of just the casts

Handheld_days <- Handhelds1 |>
  select(Reservoir, Date, DateTime, Year)|>
  distinct(Reservoir, Date, .keep_all = TRUE)

# GEt the month and date for easier graphing. This is not the correct date.   
  Handheld_days$MonthDay <- format(Handheld_days$Date,"%m-%d")
  Handheld_days$MDay <- as.Date(Handheld_days$MonthDay, "%m-%d")
  

EDI_CTD_Handhelds <- EDI_CTD %>% 
  select(Reservoir, Site, DateTime, Depth_m, Temp_C, DO_mgL, Date, Year) %>% 
  rbind(Handhelds1) %>% 
  na.omit() %>% 
  arrange(Date)
...
water_temp <- data%>%
  ...
  geom_point(data = sample_dates1, aes(x = MDay, y = 0.1, z = NULL), pch = '|', size = 2, color = "black") + #to mark cast dates 
  geom_point(data = sample_dates2, aes(x = MDay, y = 0.1, z = NULL), pch = '|', size = 2, color = "black") +
...
wer <- data.frame(
  data= c(rep("EDI_CTD_Handhelds",12)),
  var = c(rep("Temp_C",12)),
...
make_heat_map(
  data= temp_df,
  sample_dates1 = CTD_casts,
  sample_dates2 = Handheld_days,
  turnovers = turnover, 
  y_lim = c(-1, 30),
  custom_heatmap_palette = blue2green2red(60),
  y_units = "Water temperature (°C)",
  output_file = "figures/SedTraps_waterTemp_2018_2023.jpeg"
)
...
make_heat_map(
  ...
  output_file = "figures/SedTraps_DO_2018_2023.jpeg"
)
...
```{r pH set up}
CTD_pH <- EDI_CTD %>% 
  select(Reservoir, Site, DateTime, Depth_m, pH, Date, Year) %>% 
  na.omit()
...
# GEt the month and date for easier graphing. This is not the correct date.   
  CTD_pH_casts$MDay <- format(CTD_pH_casts$DateTime,"%m-%d")
  CTD_pH_casts$MDay <- as.Date(CTD_pH_casts$MDay, "%m-%d")
...
pH_Handhelds <- Handhelds %>% 
  mutate(Date = date(DateTime),
         Year = year(DateTime)) %>%
  select(Reservoir, Site, DateTime, Depth_m, pH, Date, Year) %>%
  na.omit() %>% 
  filter(Date %!in% CTD_pH_days$Date) %>% 
  #fixing leap day issue, shifting sample date by one day
  mutate(Date = if_else(Date == as.Date('2020-02-29'), 
                          as.Date('2020-02-28'), Date))

CTD_Handhelds_pH <- CTD_pH %>% 
  rbind(pH_Handhelds) %>% 
  na.omit() %>% 
  arrange(Date)

Handheld_pH_days <- pH_Handhelds |>
  select(Reservoir, Date, DateTime, Year)|>
  distinct(Reservoir, Date, .keep_all = TRUE)

# GEt the month and date for easier graphing. This is not the correct date.   
  Handheld_pH_days$MonthDay <- format(Handheld_pH_days$Date,"%m-%d")
  Handheld_pH_days$MDay <- as.Date(Handheld_pH_days$MonthDay, "%m-%d")
```
...
wer <- data.frame(
  data= c(rep("CTD_Handhelds_pH",12)),
  var = c(rep("pH",12)),
  Res = c(rep("FCR",6), rep("BVR",6)),
  Years = c(rep(2018:2023,2))
)
...
make_pH_heat_map(
  data= temp_df,
  sample_dates1 = CTD_pH_casts,
  sample_dates2 = Handheld_pH_days,
  turnovers = turnover, 
  y_lim = c(5.5, 10.5),
  custom_heatmap_palette = rev(blue2green2red(60)),
  y_units = "pH",
  output_file = "figures/SedTraps_pH_2018_2023.jpeg"
)
...
```{r turbidity set up}
CTD_turb <- EDI_CTD %>% 
  select(Reservoir, Site, DateTime, Depth_m, Turbidity_NTU, Date, Year) %>% 
  na.omit() %>% 
  filter(Turbidity_NTU <= 100)

CTD_turb_casts <- CTD_turb |>
  select(Reservoir, DateTime, Year)|>
  mutate(Date = date(DateTime)) %>% 
  distinct(Reservoir, Date, .keep_all = TRUE)

# GEt the month and date for easier graphing. This is not the correct date.   
  CTD_turb_casts$MDay <- format(CTD_turb_casts$DateTime,"%m-%d")
  CTD_turb_casts$MDay <- as.Date(CTD_turb_casts$MDay, "%m-%d")
```

```{r turbidity heat map}
...
wer <- data.frame(
  data= c(rep("CTD_turb",12)),
  var = c(rep("Turbidity_NTU",12)),
  Res = c(rep("FCR",6), rep("BVR",6)),
  Years = c(rep(2018:2023,2))
)
...
make_turb_heat_map(
  data= temp_df,
  sample_dates1 = CTD_turb_casts,
  turnovers = turnover, 
  y_lim = c(0, 100),
  custom_heatmap_palette = blue2green2red(60),
  y_units = "Turbidity (NTU)",
  output_file = "figures/SedTraps_turbidity_2018_2023.jpeg"
)
